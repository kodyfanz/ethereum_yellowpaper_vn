...
Cây Merkle Patricia sửa đổi (trie) cung cấp một cấu trúc dữ liệu bền để ánh xạ giữa dữ liệu nhị phân có độ dài tùy ý (các mảng byte). Nó được định nghĩa dưới dạng một cấu trúc dữ liệu có thể thay đổi để ánh xạ giữa các đoạn nhị phân 256 bit và dữ liệu nhị phân có độ dài tùy ý, thường được thực hiện dưới dạng một cơ sở dữ liệu. Lõi của trie, và yêu cầu duy nhất của nó trong kịch bản đặc tả giao thức, là cung cấp một giá trị duy nhất xác định một bộ cặp khóa-giá trị (key-value) đã cho, có thể là một dãy 32 byte hoặc dãy byte trống. Nó được xem xét khi triển khai để lưu trữ và duy trì cấu trúc của trie một cách hiệu quả và sự hiện thực hóa hiệu quả của giao thức.

Quy ước, chúng ta giả sử giá trị đầu vào $\mathfrak{I}$, một tập hợp chứa các cặp dãy byte có khóa (key) duy nhất:
\begin{equation}
\mathfrak{I} = \{ (\mathbf{k}_0 \in \mathbb{B}, \mathbf{v}_0 \in \mathbb{B}), (\mathbf{k}_1 \in \mathbb{B}, \mathbf{v}_1 \in \mathbb{B}), ... \}
\end{equation}

Khi xem xét một dãy như vậy, chúng ta sử dụng ký hiệu chỉ số dưới dạng số phổ biến để tham chiếu đến một bộ (tuple) khóa hoặc giá trị, như sau:
\begin{equation}
\forall I \in \mathfrak{I}: I \equiv (I_0, I_1)
\end{equation}

Bất kỳ loạt byte nào cũng có thể được trình bày là một loạt các nữa byte/4 bít (nibble), theo ghi chú kiểu endian cụ thể; Ở đây chúng ta giả sử Big-Endian. Do đó:
\begin{eqnarray}
...
Chúng ta cũng giả định có một hàm $n$, là hàm giới hạn số nút của cây trie. Khi tạo một nút, chúng ta sử dụng RLP để mã hóa cấu trúc. Nhằm giảm độ phức tạp về lưu trữ, chúng ta lưu trữ các nút mà RLP tạo ra có độ dài ít hơn 32 byte một cách trực tiếp; đối với những nút có kích thước lớn hơn, chúng ta khẳng định sự biết trước về dãy byte mà hash Keccak-256 của nó đánh giá đến tham chiếu của chúng ta. Do đó, chúng ta định nghĩa dựa trên $c$, hàm tổ hợp nút:
\begin{equation}
...
Một cách tương tự như cây radix, khi trie được duyệt từ gốc đến lá, ta có thể xây dựng một cặp key-value duy nhất. Key được tích luỹ qua quá trình duyệt, thu thập một nibble duy nhất từ mỗi nút nhánh (giống như cây radix). Khác với cây radix, trong trường hợp nhiều key chia sẻ cùng một tiền tố hoặc trong trường hợp chỉ có một key có một hậu tố duy nhất thì hai nút tối ưu hóa được cung cấp. Do đó, trong quá trình duyệt, có thể có thể lấy được nhiều nibble từ mỗi loại nút khác nhau, bao gồm nút mở rộng và nút lá. Có ba loại nút trong trie:

\begin{description}
\item[Leaf (Lá):] Một cấu trúc hai phần tử, trong đó phần tử đầu tiên tương ứng với các nibbles trong key mà chưa được tính đến bởi việc tích luỹ của keys và các nhánh duyệt từ gốc. Phương pháp mã hóa hex-prefix được sử dụng và tham số thứ hai cho hàm phải là 1.

\item[Extension (Mở rộng):] Một cấu trúc hai phần tử, phần tử đầu tiên tương ứng với một loạt các nibbles có kích thước lớn hơn một, được chia sẻ bởi ít nhất hai key khác nhau sau khi tích lũy các keys của các nibbles và các keys của nhánh duyệt từ gốc. Phương pháp mã hóa hex-prefix được sử dụng và tham số thứ hai cho hàm phải là 0.

\item[Branch (Nhánh):] Một cấu trúc 17 phần tử, trong đó 16 phần tử đầu tiên tương ứng với mỗi giá trị nibble có thể có cho các key tại điểm này trong quá trình duyệt qua nó. Mục thứ 17 được sử dụng trong trường hợp này là một nút kết thúc (terminator node), và do đó một key kết thúc tại điểm này trong quá trình duyệt qua nó.
\end{description}

Một nhánh chỉ được sử dụng khi cần thiết; không có nút nhánh nào có thể tồn tại chỉ chứa một mục khác không. Chúng ta có thể định nghĩa cấu trúc này một cách chính thức bằng hàm tổ hợp cấu trúc $c$.
\begin{equation}
